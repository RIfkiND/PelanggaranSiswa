...
<div class="w-full px-6 py-6 mx-auto">
    ...
    <div class="flex flex-wrap -mx-3">
        ...
        @php
        //hari
        $siswaPerhari = $siswas->groupBy(function ($student) {
        return $student->created_at->format('Y-m-d');
        });

        $totalsiswaPerHari = $siswaPerhari->map(function ($students) {
        return $students->count();
        });

        $percentages = [];
        $previousTotal = null;
        foreach ($totalsiswaPerHari as $day => $totalsiswa) {
        if ($previousTotal !== null) {
        $percentages[$day] = ($totalsiswa - $previousTotal) / $previousTotal * 100;
        }
        $previousTotal = $totalsiswa;
        }
        //minggu
        $siswaPerMinggu = collect($siswas)->groupBy(function ($siswa) {
        return $siswa->created_at->startOfWeek()->format('Y-m-d');
        });

        $totalSiswaPerMinggu = $siswaPerMinggu->map(function ($siswas) {
        return $siswas->count();
        });

        $tanggalTerbaru = $siswaPerMinggu->keys()->last();
        $persentase = [];
        $totalSebelumnya = null;
        foreach ($totalSiswaPerMinggu as $minggu => $totalSiswa) {
        if ($totalSebelumnya !== null) {
        $persentase[$minggu] = ($totalSiswa - $totalSebelumnya) / $totalSebelumnya * 100;
        }
        $totalSebelumnya = $totalSiswa;
        }
                        ...
                        <div class="flex-none w-2/3 max-w-full px-3">
                            <div>
                                ...
                                <p class="mb-0 dark:text-white dark:opacity-60">
                                    ...
                                    <span class="text-sm font-bold leading-normal text-emerald-500">+{{ number_format($percentages[$day], 2) }}%</span>
                                    @elseif($percentages[$day] < 0) <span class="text-sm font-bold leading-normal text-red-500">{{ number_format($percentages[$day], 2) }}%</span>
                                        @else
                                        <span class="text-sm font-bold leading-normal text-gray-500">Tidak ada perubahan</span>
                                        @endif
                                        @endif
                                        Dari Kemarin
                                </p>
            ...
            <div class="relative flex flex-col min-w-0 break-words bg-white shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                <div class="flex-auto p-4">
                    <div class="flex flex-row -mx-3">
                        <div class="flex-none w-2/3 max-w-full px-3">
                            <div>
                                <p class="mb-0 font-sans text-sm font-semibold leading-normal uppercase dark:text-white dark:opacity-60">Analisis Perminggu</p>
                                <h5 class="mb-2 font-bold dark:text-white">Total {{ $totalSiswaPerMinggu->last() }} Siswa</h5>
                                <p class="mb-0 dark:text-white dark:opacity-60">
                                    @if(isset($persentase[$tanggalTerbaru]))
                                    @if($persentase[$tanggalTerbaru] > 0)
                                    <span class="text-sm font-bold leading-normal text-emerald-500">+{{ number_format($persentase[$tanggalTerbaru], 2) }}%</span>
                                    @elseif($persentase[$tanggalTerbaru] < 0) <span class="text-sm font-bold leading-normal text-red-500">{{ number_format($persentase[$tanggalTerbaru], 2) }}%</span>
                                        @else
                                        <span class="text-sm font-bold leading-normal text-gray-500">Tidak ada perubahan</span>
                                        @endif
                                        @endif
            ...
            <div class="relative flex flex-col min-w-0 break-words bg-white shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                <div class="flex-auto p-4">
                    <div class="flex flex-row -mx-3">
                        <div class="flex-none w-2/3 max-w-full px-3">
                            <div>
                                <p class="mb-0 font-sans text-sm font-semibold leading-normal uppercase dark:text-white dark:opacity-60">Analisis PerBulan</p>
            ...
            <div class="relative flex flex-col min-w-0 break-words bg-white shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                <div class="flex-auto p-4">
                    <div class="flex flex-row -mx-3">
                        <div class="flex-none w-2/3 max-w-full px-3">
                            <div>
                                <p class="mb-0 font-sans text-sm font-semibold leading-normal uppercase dark:text-white dark:opacity-60">Analisis Persemester</p>
                            ...
                            <tr>
                                <td class="p-2 align-middle bg-transparent border-b w-3/10 whitespace-nowrap dark:border-white/40">
                                    <div class="flex items-center px-2 py-1">
                                        ...
                                        <div class="ml-6">
                                            <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-60">Kelas:</p>
                                            <h6 class="mb-0 text-sm leading-normal dark:text-white">{{ $k->name }}</h6>
                                        </div>
                                    </div>
                                </td>
                                @php
                                $ringan = $k->siswa()->whereHas('kejadian', function ($query) {
                                $query->where('category_id', 1);
                                })->count();

                                $sedang = $k->siswa()->whereHas('kejadian', function ($query) {
                                $query->where('category_id', 2);
                                })->count();

                                $berat = $k->siswa()->whereHas('kejadian', function ($query) {
                                $query->where('category_id', 3);
                                })->count();
                                @endphp
                                <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap dark:border-white/40">
                                    <div class="text-center">
                                        <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-60">Pelanggaran ringan:</p>
                                        <h6 class="mb-0 text-sm leading-normal dark:text-white">{{ $ringan }}</h6>
                                    </div>
                                </td>
                                <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap dark:border-white/40">
                                    <div class="text-center">
                                        <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-60">Pelanggaran Sedang:</p>
                                        <h6 class="mb-0 text-sm leading-normal dark:text-white">{{ $sedang }}</h6>
                                    </div>
                                </td>
                                <td class="p-2 text-sm leading-normal align-middle bg-transparent border-b whitespace-nowrap dark:border-white/40">
                                    <div class="flex-1 text-center">
                                        <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-60">Pelanggaran Berat:</p>
                                        <h6 class="mb-0 text-sm leading-normal dark:text-white">{{ $berat }}</h6>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
                ...
                <div class="overflow-x-auto">
                    <table class="items-center w-full mb-4 align-top border-collapse border-gray-200 dark:border-white/40">
                        <tbody>
                            ...
                            $ringan += $siswa->kejadian()->where('category_id', 1)->count();
                            $sedang += $siswa->kejadian()->where('category_id', 2)->count();
                            $berat += $siswa->kejadian()->where('category_id', 3)->count();
                            }
                            }
                            @endphp
                            ...
                            <tr>
                                <td class="p-2 align-middle bg-transparent border-b w-3/10 whitespace-nowrap dark:border-white/40">
                                    <div class="flex items-center px-2 py-1">
                                        ...
                                        <div class="ml-6">
                                            <p class="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-60">Jurusan:</p>
                                            <h6 class="mb-0 text-sm leading-normal dark:text-white">{{ $j->name }}</h6>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap dark:border-white/40">
            ...
            <div class="flex flex-wrap items-center -mx-3 lg:justify-between">
                <div class="w-full max-w-full px-3 mt-0 mb-6 shrink-0 lg:mb-0 lg:w-1/2 lg:flex-none">
                    <div class="text-sm leading-normal text-center text-slate-500 lg:text-left">
                        ...
                        <a href="senja anak baik" class="font-semibold text-slate-700 dark:text-white" target="_blank">Tim SPYWARE</a>
                        for smkn 1 ciamis
                    </div>
                </div>
