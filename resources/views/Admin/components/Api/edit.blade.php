...
<dialog id="form_edit" class="modal">
    <div class="modal-box w-11/12 max-w-5xl">
        <form id="editForm" method="POST" enctype="multipart/form-data" action="">
            @method('PUT')
            @csrf
            <h3 class="font-bold text-xl text-right align-self-center flex">{{ __('EDIT FORM') }}</h3>
            <hr class="w-full h-1 my-8 bg-gray-600 border-0 rounded dark:bg-gray-700">

            <!-- Update input names and IDs -->
            <input type="text" name="name" id="edit_name" placeholder="Type here" class="input input-bordered w-full " />

            <!-- Assuming 'pelanggaran_score' is the correct attribute name -->
            <input type="number" name="pelanggaran_score" id="edit_pelanggaran_score" placeholder="Score" class="input input-bordered w-full mt-5" />

            <select name="category_id" id="edit_category_id" class="select select-bordered w-full  mt-5">
            ...
            <textarea name="penjelasan" id="edit_penjelasan" class="textarea textarea-bordered w-full mt-5" placeholder="Penjelasan"></textarea>
